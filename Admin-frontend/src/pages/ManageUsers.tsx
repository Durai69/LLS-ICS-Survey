...
const ManageUsers = () => {
  ...
  const fetchUsers = useCallback(async () => {
    ...
    setError(null);
    try {
      const response = await axios.get<User[]>(`${API_BASE_URL}/users`, { withCredentials: true });
  ...
  const handleAddNewDepartment = async (deptName: string) => {
    try {
      const response = await axios.post<{ id: number }>(`${API_BASE_URL}/departments`, { name: deptName }, { withCredentials: true });
  ...
  const handleDeleteUser = async () => {
    ...
    try {
      const response = await axios.delete(`${API_BASE_URL}/users/${userToDelete.id}`, { withCredentials: true });
